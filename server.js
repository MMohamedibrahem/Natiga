...
const GZ_PATH = path.join(__dirname, 'students.db.gz');
const PUBLIC_DIR = path.join(__dirname, 'public');

// Automatically decompress database on startup if the raw .db file is missing (e.g. on Render/Railway)
if (!fs.existsSync(DB_PATH)) {
  if (fs.existsSync(GZ_PATH)) {
    console.log('students.db not found. Decompressing students.db.gz... Please wait...');
    try {
      const gzipBuffer = fs.readFileSync(GZ_PATH);
      const dbBuffer = zlib.gunzipSync(gzipBuffer);
      fs.writeFileSync(DB_PATH, dbBuffer);
      console.log('Decompression complete! students.db created successfully.');
    } catch (gzError) {
      console.error('Failed to decompress database on startup:', gzError);
      process.exit(1);
    }
  } else {
    console.error('Critical Error: Neither students.db nor students.db.gz was found!');
    process.exit(1);
  }
}
